...
const main = async () => {
  ...
  const logFilePath = options['--log-file'] || process.env.MCPCTL_LOG_FILE || undefined;
  ...
  try {
    ...
    if (error.code === 'ENOENT') {
      console.error('Daemon is not running, trying to start it by running `mcpctl daemon start`');
    } else {
      if (!verbose) {
