...
export const sessionConnectCommand = async (app: App, argv: string[]) => {
  ...
  if (options['--help']) {
    ...
    logger.info('Usage: mcp session connect --server <server> --command <command> [--command-base64 <command-base64>]');
    logger.info('Options:');
    logger.info('  -s, --server: Server name');
    logger.info('  -c, --command: Command to run');
    logger.info('  -c64, --command-base64: Command to run (base64 encoded, since cursor parses mcp.json wrong)');
  ...
  if (!command) {
    ...
    throw new ValidationError('Error: Something went wrong with command decoding.');
  }
  const profileName = options['--profile'] || 'default';
  const env = options['--env'] || [];
  const secret = options['--secret'] || [];
  let envMap: ServerEnvConfig = await getAndUpdateServerEnv(app, profileName, serverName, env);
...
async function getAndUpdateServerEnv(
  ...
  const profileEnvConfig = await profileService.getProfileEnvForServer(profileName, serverName);
  ...
  await profileService.upsertProfileEnvForServer(profileName, serverName, newEnvMap);
