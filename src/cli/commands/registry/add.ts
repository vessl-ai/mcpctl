...
export const addCommand = async (app: App, argv: string[]) => {
  ...
  if (!Object.values(RegistryType).includes(type as RegistryType)) {
    logger.error(`Invalid registry type. Must be one of: ${Object.values(RegistryType).join(', ')}`);
    throw new ValidationError(`Invalid registry type. Must be one of: ${Object.values(RegistryType).join(', ')}`);
  }
  ...
  try {
    ...
    logger.error(`Failed to add registry: ${error instanceof Error ? error.message : 'Unknown error'}`);
