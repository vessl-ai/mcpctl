...
import os from 'os';
import { McpServerInstallConfig, McpServerType } from '../../core/lib/types/mcp-server';
...
export const installCommand = async (app: App, argv: string[]) => {
  ...
  if (serverName === '') {
    logger.error('Error: Server name is required. Use -n or --server-name option.');
  ...
  const logFileKey = `${serverName}-${serverType}-${profile ? profile : ''}-${sanitizedCommand}`;
  ...
  if (client.length > 0) {
    for (const c of client) {
      const installedServerConfig = await clientService.installMcpServerToClient(
        clientService.getClient(c),
        serverInstallConfig
      );
      console.log(chalk.green(`MCP server installed successfully to ${c}.`));
      if (c === 'claude') {
        console.log(chalk.green('Please restart Claude Desktop to use the new MCP server.'));
      }
