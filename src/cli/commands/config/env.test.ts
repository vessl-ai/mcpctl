...
describe('envSetCommand', () => {
  ...
  it('should set shared environment variables successfully', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('✅ Shared environment variables updated successfully!');
  });
  ...
  it('should set profile environment variables successfully', async () => {
    ...
    expect(mockProfileService.upsertProfileEnvForServer).toHaveBeenCalledWith('testProfile', 'testServer', {
      KEY: 'value',
    });
  ...
  it('should fail when no environment variables are provided', async () => {
    ...
    expect(mockLogger.error).toHaveBeenCalledWith('Error: At least one environment variable is required (--env, -e)');
  });
  ...
  it('should fail when using shared with profile', async () => {
    ...
    await expect(envSetCommand(mockApp, argv)).rejects.toThrow();
    expect(mockLogger.error).toHaveBeenCalledWith('Error: Cannot use --shared with --profile or --server');
  });
});
...
describe('envGetCommand', () => {
  ...
  it('should get shared environment variables successfully', async () => {
    ...
    expect(mockConfigService.getConfigSection).toHaveBeenCalledWith('sharedEnv');
  ...
  it('should get profile environment variables successfully', async () => {
    ...
    expect(mockProfileService.getProfileEnvForServer).toHaveBeenCalledWith('testProfile', 'testServer');
  ...
  it('should fail when profile not found', async () => {
    ...
    expect(mockLogger.error).toHaveBeenCalledWith("Profile 'nonexistentProfile' not found");
  });
  ...
  it('should fail when server not found in profile', async () => {
    ...
    await expect(envGetCommand(mockApp, argv)).rejects.toThrow();
    expect(mockLogger.error).toHaveBeenCalledWith("Server 'nonexistentServer' not found in profile 'testProfile'");
  });
});
...
describe('envListCommand', () => {
  ...
  it('should list all environment variables when no options provided', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('------------------------------');
    expect(consoleLogSpy).toHaveBeenCalledWith('  - SHARED_KEY1: shared_value1');
    expect(consoleLogSpy).toHaveBeenCalledWith('  - SHARED_KEY2: shared_value2');

    // Check profile1/server1 section
    expect(consoleLogSpy).toHaveBeenCalledWith('\n📌 profile1/server1:');
    expect(consoleLogSpy).toHaveBeenCalledWith('-'.repeat('profile1'.length + 'server1'.length + 4));
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('\n📌 profile2/server2:');
    expect(consoleLogSpy).toHaveBeenCalledWith('-'.repeat('profile2'.length + 'server2'.length + 4));
  ...
  it('should show message when no shared environment variables exist', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('\n🔍 Shared environment variables:');
    expect(consoleLogSpy).toHaveBeenCalledWith('==============================');
    expect(consoleLogSpy).toHaveBeenCalledWith('  No shared environment variables set');
  });
  ...
  it('should show message when no profile environment variables exist', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('\n🔍 Environment variables for testProfile/testServer:');
    expect(consoleLogSpy).toHaveBeenCalledWith('==========================================');
    expect(consoleLogSpy).toHaveBeenCalledWith('  No environment variables set');
  });
