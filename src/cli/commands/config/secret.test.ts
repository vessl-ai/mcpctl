...
import { App } from '../../app';
import { secretGetCommand, secretListCommand, secretRemoveCommand, secretSetCommand } from './secret';
...
describe('Secret Commands', () => {
  ...
  describe('secretSetCommand', () => {
    it('should set shared secrets when --shared flag is used', async () => {
      const argv = ['--shared', '--entry', 'KEY1=value1', '--entry', 'KEY2=value2'];
      ...
      expect(consoleSpy).toHaveBeenCalledWith('✅ Shared secret updated successfully!');
    });
    ...
    it('should set profile secrets when profile and server are specified', async () => {
      ...
      const argv = ['--profile', 'myProfile', '--server', 'myServer', '--entry', 'KEY1=value1'];
      await secretSetCommand(app, argv);

      expect(profileService.upsertProfileSecretsForServer).toHaveBeenCalledWith('myProfile', 'myServer', {
        KEY1: 'value1',
      });
      expect(consoleSpy).toHaveBeenCalledWith('✅ Secret updated successfully for myProfile/myServer!');
    });

    it('should fail when using both --shared and --profile flags', async () => {
      const argv = ['--shared', '--profile', 'myProfile', '--entry', 'KEY=value'];
      await expect(secretSetCommand(app, argv)).rejects.toThrow(
        new ValidationError('Cannot use --shared with --profile or --server')
      );

      expect(logger.error).toHaveBeenCalledWith('Error: Cannot use --shared with --profile or --server');
    });
  });
  ...
  describe('secretGetCommand', () => {
    it('should get all shared secrets when --shared flag is used', async () => {
      ...
      secretService.getSharedSecret.mockImplementation(async key => (key === 'KEY1' ? 'value1' : 'value2'));
    ...
    it('should get specific profile secret when profile, server and key are specified', async () => {
      ...
      const argv = ['--profile', 'myProfile', '--server', 'myServer', '--key', 'KEY1'];
      await secretGetCommand(app, argv);
  ...
  describe('secretRemoveCommand', () => {
    it('should remove shared secret when --shared flag is used', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("✅ Shared secret 'KEY1' removed successfully!");
    });
    ...
    it('should remove profile secret when profile and server are specified', async () => {
      ...
      expect(profileService.removeProfileSecret).toHaveBeenCalledWith('myProfile', 'myServer', 'KEY1');
      expect(consoleSpy).toHaveBeenCalledWith("✅ Secret 'KEY1' removed successfully from myProfile/myServer!");
    });

    it('should fail when key is not provided', async () => {
      const argv = ['--shared'];
      await expect(secretRemoveCommand(app, argv)).rejects.toThrow(new ValidationError('Secret key is required'));

      expect(logger.error).toHaveBeenCalledWith('Error: Secret key is required (--key, -k)');
    });
  });
  ...
  describe('secretListCommand', () => {
    ...
    it('should list profile secrets when profile and server are specified', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('=====================================');
      expect(consoleSpy).toHaveBeenCalledWith('  - KEY1: secret1 (First secret)');
      expect(consoleSpy).toHaveBeenCalledWith('  - KEY2: secret2 (Second secret)');
    });
    ...
    it('should list all secrets when no flags are provided', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('\n🔐 Shared secrets:');
      expect(consoleSpy).toHaveBeenCalledWith('----------------');
      expect(consoleSpy).toHaveBeenCalledWith('  - SHARED1: First shared secret');

      // Verify profile secrets section
      expect(consoleSpy).toHaveBeenCalledWith('\n🔐 profile1/server1:');
      expect(consoleSpy).toHaveBeenCalledWith('=============');
      expect(consoleSpy).toHaveBeenCalledWith('  - KEY1: secret1 (Profile 1 secret)');
    });

    it('should fail when profile not found', async () => {
      profileService.getProfile.mockReturnValue(undefined);
      const argv = ['--profile', 'nonexistentProfile', '--server', 'testServer'];

      await expect(secretListCommand(app, argv)).rejects.toThrow(
        new ResourceNotFoundError("Profile 'nonexistentProfile' not found")
      );
      expect(logger.error).toHaveBeenCalledWith("Profile 'nonexistentProfile' not found");
    });
    ...
    it('should fail when server not found in profile', async () => {
      ...
      profileService.getProfile.mockReturnValue(mockProfile);
      const argv = ['--profile', 'testProfile', '--server', 'nonexistentServer'];

      await expect(secretListCommand(app, argv)).rejects.toThrow(
        new ResourceNotFoundError("Server 'nonexistentServer' not found in profile 'testProfile'")
      );
      expect(logger.error).toHaveBeenCalledWith("Server 'nonexistentServer' not found in profile 'testProfile'");
    });
  });
});
