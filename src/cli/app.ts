...
import { SecretService, newSecretService } from '../core/services/secret/secret-service';
import { SecretStore, newKeychainSecretStore } from '../core/services/secret/secret-store';
...
class App {
  ...
  private async initializeDependencies({ logLevel = LogLevel.INFO, logger }: AppOptions): Promise<void> {
  ...
  private registerServerService(logger: Logger) {
    this.container.register<ServerService>(
      'serverService',
      newServerService(this.container.get<Logger>('Logger').withContext('ServerService'))
    );
  ...
  private registerSessionManager(logger: Logger) {
    this.container.register<SessionManager>(
      'sessionManager',
      newSessionManager(this.container.get<Logger>('Logger').withContext('SessionManager'))
    );
    logger.debug('SessionManager registered');
  }

  private registerProfileService(logger: Logger) {
    this.container.register<ProfileStore>('profileStore', newFileProfileStore());
  ...
  private registerSecretService(logger: Logger) {
    this.container.register<SecretStore>('secretStore', newKeychainSecretStore());
  ...
  private registerRegistryService(logger: Logger) {
    this.container.register<RegistryDefStore>(
      'registryDefStore',
      newConfigRegistryDefStore(this.container.get<ConfigService>('configService'))
    );
    logger.debug('RegistryDefStore registered');
    this.container.register<RegistryProviderFactory>('registryProviderFactory', newRegistryProviderFactory());
  ...
  private registerConfigService(logger: Logger) {
    this.container.register<ConfigStore>(
      'configStore',
      newFileConfigStore(this.container.get<Logger>('Logger').withContext('ConfigStore'))
    );
  ...
  public getRegistryService(): RegistryService {
    const registryService = this.container.get<RegistryService>('registryService');
    if (!registryService) {
