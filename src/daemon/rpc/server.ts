...
export class RPCServer {
  ...
  private setupConnection() {
    try {
      ...
      this.connection = createMessageConnection(this.transport.reader, this.transport.writer);
  ...
  private setupHandlers() {
    if (!this.connection) {
      const err = new Error('Cannot setup handlers: Connection not established');
    ...
    this.connection.onRequest(Instance.StartRequest.type, async ({ config }) => {
    ...
    this.connection.onRequest(Instance.StopRequest.type, async ({ instanceId }) => {
    ...
    this.connection.onRequest(Instance.GetRequest.type, async ({ instanceId }) => {
    ...
    this.connection.onNotification(Instance.StatusNotification.type, async ({ instanceId, status }) => {
  ...
  public listen(): void {
    ...
    if ('_server' in this.transport) {
      ...
      this.logger.debug('Client transport detected, setting up connection immediately');
      this.setupConnection();
    }
  }
