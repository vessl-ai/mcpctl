...
import { newSecretService, SecretService } from '../core/services/secret/secret-service';
import { newKeychainSecretStore, SecretStore } from '../core/services/secret/secret-store';
...
export class DaemonApp {
  ...
  private registerConfigService(logger: Logger) {
    this.container.register<ConfigStore>('configStore', newFileConfigStore(logger.withContext('ConfigStore')));
  ...
  private async createRpcServer(): Promise<void> {
    ...
    this.rpcServer = new RPCServer(transport, this.getInstanceManager(), logger);
