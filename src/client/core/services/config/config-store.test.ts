...
describe('FileConfigStore', () => {
  ...
  beforeEach(() => {
    ...
    configStore = new FileConfigStoreImpl(testConfigPath);
  });
  ...
  describe('constructor', () => {
    it('should create config file if it does not exist', () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(testConfigPath, JSON.stringify({}));
    });
  ...
  describe('getConfig', () => {
    it('should read and parse config file', () => {
      ...
      expect(logger.verbose).toHaveBeenCalledWith(`Loading config from ${testConfigPath}`);
      expect(logger.verbose).toHaveBeenCalledWith(`Loaded config: ${JSON.stringify(mockConfig, null, 2)}`);
    });
  ...
  describe('saveConfig', () => {
    it('should write config to file', () => {
      ...
      expect(logger.verbose).toHaveBeenCalledWith(`Saving config to ${testConfigPath}, ${JSON.stringify(mockConfig, null, 2)}`);
      expect(logger.verbose).toHaveBeenCalledWith(`Saved config to ${testConfigPath}`);
    });
