...
export interface SecretService {
  ...
  setSharedSecrets(secrets: Record<string, string>): Promise<void>;
...
export class SecretServiceImpl implements SecretService {
  ...
  async setSharedSecrets(secrets: Record<string, string>): Promise<void> {
    const config = this.configService.getConfig();
  ...
  async resolveEnv(
    ...
    // null이 아닌 값들만 병합 (프로필 시크릿이 공용 시크릿 오버라이드)
    [...sharedSecretEntries, ...profileSecretEntries]
