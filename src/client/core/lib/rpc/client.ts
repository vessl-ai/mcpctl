...
export class DaemonRPCClient {
    ...
    private constructor(connection: MessageConnection, logger: Logger) {
        ...
        this.connection.onError((error) => {
            ...
            if (error.code === "ENOENT") {
                this.logger.error('Daemon is not running, trying to start it by running `mcp daemon start`');
            }
    ...
    async startInstance(config: RunConfig): Promise<McpServerInstance> {
        ...
        return await this.connection.sendRequest(Instance.StartRequest.type, { config });
    }
