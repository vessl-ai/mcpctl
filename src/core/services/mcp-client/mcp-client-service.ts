...
export interface McpClientService {
  ...
  installMcpServerToClient(client: McpClient, serverInstallConfig: McpServerInstallConfig): Promise<McpServerConfig>;
}
...
export class McpClientServiceImpl implements McpClientService {
  ...
  generateMcpServerConfig(
    ...
    switch (serverInstallConfig.type) {
      case 'stdio':
        ...
        const [command, args] = this.generateCommand(serverInstallConfig, options);
  ...
  async installMcpServerToClient(
    ...
    if (serverInstallConfig.secrets && Object.keys(serverInstallConfig.secrets).length > 0) {
      ...
      if (serverInstallConfig.profile) {
        ...
        secretRef = await this.secretService.setSharedSecrets(serverInstallConfig.secrets || {});
      }
    ...
    switch (client.type) {
      case McpClientType.CLAUDE:
        serverConfig = await this.installMcpServerToClaude(client, serverInstallConfig);
      case McpClientType.CURSOR:
        serverConfig = await this.installMcpServerToCursor(client, serverInstallConfig);
    }
  ...
  private getOrGenerateServerName(serverInstallConfig: McpServerInstallConfig): string {
  ...
  private generateCommand(
    ...
    const args = ['session', 'connect', '--server', serverInstallConfig.serverName];
    if (options.useBase64Command) {
      args.push('--command-base64', Buffer.from(originalCommand).toString('base64'));
    } else {
