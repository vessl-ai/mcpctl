...
describe('SearchService', () => {
  ...
  describe('searchByRegistryAndName', () => {
    it('should find entry by exact name in specific registry', async () => {
      const registryProvider = registryService.getRegistryProvider('test-registry');
      (registryProvider.findEntryByName as jest.Mock).mockResolvedValue(mockRegistryEntry);

      const result = await searchService.searchByRegistryAndName('test-registry', 'test-entry');

      expect(registryService.getRegistryProvider).toHaveBeenCalledWith('test-registry');
      expect(registryProvider.findEntryByName).toHaveBeenCalledWith('test-entry', undefined);
  ...
  describe('searchByQueryForRegistry', () => {
    it('should find entries by query in specific registry', async () => {
      const registryProvider = registryService.getRegistryProvider('test-registry');
      (registryProvider.findEntriesByQuery as jest.Mock).mockResolvedValue([mockRegistryEntry]);

      const result = await searchService.searchByQueryForRegistry('test-registry', 'test');

      expect(registryService.getRegistryProvider).toHaveBeenCalledWith('test-registry');
      expect(registryProvider.findEntriesByQuery).toHaveBeenCalledWith('test', undefined);
      expect(result).toEqual({ entries: [expectedSearchResultEntry] });
    });
  });
  ...
  describe('searchBySemanticQueryForRegistry', () => {
    it('should find entries by semantic query in specific registry', async () => {
      const registryProvider = registryService.getRegistryProvider('test-registry');
      (registryProvider.findEntriesBySemanticQuery as jest.Mock).mockResolvedValue([mockRegistryEntry]);

      const result = await searchService.searchBySemanticQueryForRegistry('test-registry', 'test description');

      expect(registryService.getRegistryProvider).toHaveBeenCalledWith('test-registry');
      expect(registryProvider.findEntriesBySemanticQuery).toHaveBeenCalledWith('test description', undefined);
      expect(result).toEqual({ entries: [expectedSearchResultEntry] });
    });
  });
  ...
  describe('searchBySemanticQuery', () => {
    it('should search semantically across all registries', async () => {
      ...
      const result = await searchService.searchBySemanticQuery('test description');
