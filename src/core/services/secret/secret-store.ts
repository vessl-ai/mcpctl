...
export class KeychainSecretStore implements SecretStore {
  ...
  async getSecret(profile: string, key: string): Promise<string | null> {
    return keytar.getPassword(SECRET_STORE.SERVICE_NAME, this.getSecretKey(profile, key));
  }

  async setSecret(profile: string, key: string, value: string): Promise<void> {
    await keytar.setPassword(SECRET_STORE.SERVICE_NAME, this.getSecretKey(profile, key), value);
  }

  async removeSecret(profile: string, key: string): Promise<boolean> {
    return keytar.deletePassword(SECRET_STORE.SERVICE_NAME, this.getSecretKey(profile, key));
  }
  ...
  async removeAllSecrets(profile: string): Promise<void> {
    ...
    await Promise.all(
      creds
        ...
        .map(cred => keytar.deletePassword(SECRET_STORE.SERVICE_NAME, cred.account))
    );
  }
}

// 팩토리 함수
export const newKeychainSecretStore = (): SecretStore => new KeychainSecretStore();
