...
class FileConfigStoreImpl implements ConfigStore {
  ...
  saveConfig(config: Config): void {
    this.logger.verbose(`Saving config to ${this.configPath}, ${JSON.stringify(config, null, 2)}`);
    fs.writeFileSync(this.configPath, JSON.stringify(config, null, 2));
    this.logger.verbose(`Saved config to ${this.configPath}`);
  }
}

const newFileConfigStore = (logger: Logger, configPath: string = getConfigPath()): ConfigStore => {
