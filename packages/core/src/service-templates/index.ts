...
<dict>
    ...
    <string>${path.join(
      Config.Path.LOG_PATHS.darwin,
      "daemon.error.log"
    )}</string>
    <key>StandardOutPath</key>
    <string>${path.join(Config.Path.LOG_PATHS.darwin, "daemon.log")}</string>
...
Restart=always
StandardOutput=append:${path.join(Config.Path.LOG_PATHS.linux, "daemon.log")}
StandardError=append:${path.join(
    Config.Path.LOG_PATHS.linux,
    "daemon.error.log"
  )}
...
export const SERVICE_COMMANDS = {
  darwin: {
    start: ["launchctl", "load", "-w", Config.Path.SERVICE_PATHS.darwin],
    stop: ["launchctl", "unload", "-w", Config.Path.SERVICE_PATHS.darwin],
  },
  linux: {
