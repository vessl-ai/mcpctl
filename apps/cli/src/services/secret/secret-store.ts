...
export class KeychainSecretStore implements SecretStore {
  private getSecretKey(profile: string, key: string): string {
    const namespace =
      profile === Config.Secret.SECRET_STORE.SHARED_PROFILE
        ? Config.Secret.SECRET_STORE.NAMESPACE.SHARED
        : `${Config.Secret.SECRET_STORE.NAMESPACE.PROFILE}.${profile}`;
    const sanitizedKey = key.replace(/[^a-zA-Z0-9_-]/g, "_").toLowerCase();
    return `${Config.Secret.SECRET_STORE.NAMESPACE.ROOT}.${namespace}.${sanitizedKey}`;
  }

  private isProfileSecret(account: string, profile: string): boolean {
    const expectedPrefix =
      profile === Config.Secret.SECRET_STORE.SHARED_PROFILE
        ? `${Config.Secret.SECRET_STORE.NAMESPACE.ROOT}.${Config.Secret.SECRET_STORE.NAMESPACE.SHARED}.`
        : `${Config.Secret.SECRET_STORE.NAMESPACE.ROOT}.${Config.Secret.SECRET_STORE.NAMESPACE.PROFILE}.${profile}.`;
  ...
  async getSecret(profile: string, key: string): Promise<string | null> {
    return keytar.getPassword(
      Config.Secret.SECRET_STORE.SERVICE_NAME,
