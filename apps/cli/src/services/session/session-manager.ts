...
class DefaultSessionManager implements SessionManager {
  ...
  async connect(config: RunConfig, client?: McpClientType): Promise<Session> {
    ...
    try {
      ...
      this.logger.info("Starting instance with config", {
        ...
        timestamp: new Date().toISOString(),
        rpcClientStatus: this.daemonClient ? "connected" : "undefined",
      });

      const instance = await this.daemonClient
      ...
      if (instance.status === McpServerInstanceStatus.STARTING) {
        ...
        return new Promise((resolve, reject) => {
          ...
          // this.daemonClient.onInstanceStatusChange(async (instanceId, status) => {
      ...
      const errorContext = {
        ...
        endTime: new Date().toISOString(),
        rpcAttempts,
        daemonClientStatus: this.daemonClient ? "present" : "absent",
      };
      ...
      //     await this.daemonClient.dispose();
      //   } catch (disposeError) {
  ...
  async disconnect(
    ...
    try {
      ...
      if (killInstance) {
        ...
        await this.daemonClient.stopInstance(session.instanceId);
      }
  ...
  async getStatus(sessionId: string): Promise<Session> {
    ...
    try {
      const instance = await this.daemonClient.getInstance(session.instanceId);
      if (!instance) {
...
export const newSessionManager = (
  ...
  return new DefaultSessionManager(daemonClient, logger);
};
